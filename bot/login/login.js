// set bash title
process.stdout.write("\x1b]2;Goat Bot V2 - Made by NTKhang, Updated by SaGor69x\x1b\x5c");
...
let subTitle = `SAGOR 69X BOT V2@${currentVersion} - Made by NTKhang  |  Updated by SaGorx69x`;
const subTitleArray = [];
...
const author = ("Made by NTKhang  |  Updated by SaGorx69x");
...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			log.info("BOT NICK NAME", global.GoatBot.config.nickNameBot || "SAGOR 𝕏 BOT V2");
			log.info("MAINTAINER", "Made by NTKhang  |  Updated by 𝘚𝘈𝘎𝘖𝘙");
			...
			if (global.GoatBot.config.autoRefreshFbstate == true) {
				...
				// Add extra protection: Auto refresh cookie every 1 hour
				setInterval(async () => {
					try {
			...
			console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("Project SAGOR 𝕏 Bot v2 based on GoatBot v2 by NTKhang (https://github.com/ntkhang03), updated by SaGor. Please do not sell this source code or claim it as your own. Thank you!")}\x1b[0m`);
			logColor("#f5ab00", character);
